...
def detect(frame):
    ...
    for i in np.arange(0, detections.shape[2]):
    	...
    	if confidence > args["confidence"]:
    		...
    		#print(location)
    		initState.append([int((startX + endX)/2), int((startY + endY)/2)])
    ...
    return locations, image, initState
...
locations, image, initState = detect(first_frame)
...
while True:
	...
	if not first_frame:
		...
		for i, (tracker, kalman) in enumerate(zip(trackers, kalmanfilters)):
			...
			if success:
				...
				prediction[0] = prediction[0] + initState[i][0] #
				prediction[1] = prediction[1] + initState[i][1] #


				measurement[0] = x+int(w/2) - initState[i][0] #
				measurement[1] = y+int(h/2) - initState[i][1] #
				...
				label = "X-Velocity: {0:.2f}".format(float(state[2] * 10 * (0.72/30)))
				label1 = "X-Position: {0:.2f}, X-Velocity: {1:.2f}".format(float(state[0]), float(state[2] * 10 * (0.72/30)))
	...
	if first_frame:
		...
		locations, image, initState = detect(frame)
		print(locations)
		...
		for i in range(len(locations)):
			kalmanfilters[i].measurementMatrix = np.array([[1,0,1,0],
                                                 [0,1,0,1]],np.float32)
			...
			kalmanfilters[i].processNoiseCov = 1 * np.array([[0.01,0,0,0],
                                               [0,0.01,0,0],
                                               [0,0,0.01,0],
                                               [0,0,0,0.01]],np.float32)

		for i, (startX, startY, endX, endY) in enumerate(locations):
			initBB = (startX, startY, (endX-startX)*1.05, (endY-startY)*1.05)
